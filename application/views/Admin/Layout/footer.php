...
<script>
	$(function() {
		 ...
		 */
		<?php
		$data_transaksi = $this->db->query("SELECT SUM(tot_bayar) as total_bayar, tgl_transaksi FROM `transaksi` GROUP BY tgl_transaksi")->result();
		...
		<?php
		$data_produk = $this->db->query("SELECT COUNT(id_produk) as jml, stat_produk FROM `produk` GROUP BY stat_produk")->result();
		foreach ($data_produk as $key => $value) {
			$stat[] = $value->stat_produk;
			$jml[] = $value->jml;
		}
		?>
		var doughnutPieData = {
