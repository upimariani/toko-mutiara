					...
					<div class="card-body px-3 py-4">
						<div class="d-flex justify-content-between align-items-start">
							<div class="color-card">
								<?php
								$total_tdk_laku = $this->db->query("SELECT COUNT(id_analisis) as jml, status FROM `analisis` WHERE thn_periode='2020' AND status='1'")->row();
								?>
								<a href="<?= base_url('Pemilik/cSegmentasi/detail_pertama/' . $total_tdk_laku->status) ?>">
				...
				<div class="card bg-success">
					<div class="card-body px-3 py-4">
						<div class="d-flex justify-content-between align-items-start">
							<div class="color-card">
								<?php
								$total_laku = $this->db->query("SELECT COUNT(id_analisis) as jml, status FROM `analisis` WHERE thn_periode='2020' AND status='2'")->row();
								?>
								<a href="<?= base_url('Pemilik/cSegmentasi/detail_pertama/' . $total_laku->status) ?>">
			...
			<div class="col-lg-12 grid-margin stretch-card">
				<div class="card">
					<div class="card-body">
						...
						<canvas id="segmentasi_pertama" height="150"></canvas>
					</div>
				</div>
			</div>
